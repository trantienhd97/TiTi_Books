...
| **Tổng số dòng (module)** | **12.670** dòng (toàn bộ folder `general_dashboard/`) |
| **File page chính** | `general_dashboard_page.dart` — **1.418** dòng; `dashboard_compliance_today_section.dart` — **1.423** dòng; `packages/supa_work/lib/utils/compliance_site_rate_utils.dart` — **246** dòng; `dashboard_quality_index_section.dart` — **705** dòng; `quality_index_detail_page.dart` — **1.197** dòng; `training_report_detail_page.dart` — **1.362** dòng; `dashboard_training_report_section.dart` — **874** dòng; `dashboard_home_report_filter.dart` — **114** dòng; `packages/supa_work/lib/pages/analytics/widgets/home_report_filter_sheet.dart` — **498** dòng; `site_compliance_details_list_page.dart` — **1.911** dòng; `packages/supa_work/lib/pages/analytics/site_report/widgets/compliance_site_detail_sheet.dart` |
...
| **🔵 Báo cáo** (Report) | Manager / supervisor theo dõi chỉ số hiệu suất theo site & tháng | Compliance Today, Task Progress, Quality Index, Training Report |
...
- Sau khi 3 nhóm xong, gọi `refresh()` lên 4 GlobalKey của section báo cáo (chúng tự fetch repo riêng — không nằm trong `Future.wait` chính).
...
| `DashboardComplianceTodaySection` | `dashboard_compliance_today_section.dart` | **Compliance** (report-menu-gated qua `AnalyticsPermissions.hasAnyReportPermission`). Fetch song song `WorkComplianceReportRepository.summary` + `.list` (`/rpc/work/report/compliance-report/*`). Header: icon + title "Compliance", **Filter** action (`onTapFilter`), link **Full report >** (`onTapDetail`). Body: (1) metric row 4 ô Completed/Lated/InProgress/Missed từ summary counts; (2) `_SegmentedProgressBar` 4 màu; (3) **COMPLIANCE BY SHIFT - TODAY** — `_HierarchicalSitesSection` grid 4 cột drill-down; % tile qua `compliance_site_rate_utils.dart` (leaf từ `contents[]` counts `(completed+late)/total`; cha aggregate volume-weighted). Tap leaf → `ComplianceSiteDetailSheet`; tap schedule → `_ScheduleDetailSheet`. Ngưỡng màu `[0,30)/[30,80)/[80,100]`. (4) **EVALUATION** 3 pill. Link **Xem** → `SiteComplianceDetailsListPage` (heatmap dùng cùng utility %). |
...
| `DashboardTaskSection` | `dashboard_task_section.dart` | Tasks / empty guide |
...
| 5 | **Công việc của tôi** (list) | `MobileTaskAssignmentRepository.list` | `POST /rpc/work/task-assignment/list` | `TaskAssignmentFilter`<br/>• `take = 20`<br/>• `viewCode = 'TASK_MASTER_DEFAULT'`<br/>• `assigneeOrSupporterId = <currentUserId>`<br/>• `taskAssignmentStatusId.inList = [NEW.id, DOING.id]` | `_loadWorkData()` qua `getData()` · `_refillTasks()` khi list sparse (<15) |
| 6 | **Công việc của tôi** (count) | `MobileTaskAssignmentRepository.count` | `POST /rpc/work/task-assignment/count` | Cùng `TaskAssignmentFilter` như trên (trừ `take`) | `_loadWorkData()` · `_refillTasks()` |
...
> User có thể bật/tắt từng section trong `AppSettingsPage`; các option báo cáo chỉ hiện khi user có quyền menu Báo cáo.
...
User chọn ở `AppSettingsPage` nhóm **Phần hiển thị trên Của tôi** để bật/tắt từng section và giữ icon kéo để đổi thứ tự:

| Nhóm | Section |
|------|---------|
| **Thực hiện** | Heads Up, Chấm công, Checklist, Công việc, Lộ trình, Khoá học, Quiz |
| **Báo cáo** | Compliance, Quality Index, Task Progress, Training Report |
...
- **Quyền báo cáo**: 4 tuỳ chọn báo cáo trong Settings chỉ hiện khi user có quyền menu Báo cáo (`AnalyticsPermissions.hasAnyReportPermission(paths)`). Dù user bật trong setting, từng report trên dashboard vẫn bị ẩn nếu thiếu quyền report tương ứng.
...
- **Tap toàn dòng** → `onTap` → `DashboardInspectionListWidget.onGoToDetail` → mở bottom sheet `HomeInspectionDetailPage.show` (xem chi tiết / vị trí ngoài vùng / lý do không làm).
